...
def upload_target(filename):
    ...
    collection = client.get_database(
        "prediction_service").get_collection("data")
    with open(filename) as f_target:
        for line in f_target.readlines():
            row = line.split(",")
            collection.update_one(
                {"id": row[0]}, {"$set": {"target": float(row[1])}})
    client.close()


@task
def load_reference_data(filename):
    MODEL_FILE = os.getenv('MODEL_FILE', './models/clf.bin')
    with open(MODEL_FILE, 'rb') as f_in:
        model = pickle.load(f_in)
    reference_data = pq.read_table(filename).to_pandas()
    # Create features
    reference_data['PU_DO'] = reference_data['PULocationID'].astype(
        str) + "_" + reference_data['DOLocationID'].astype(str)

    # add target column
    reference_data['target'] = reference_data.lpep_dropoff_datetime - \
        reference_data.lpep_pickup_datetime
    reference_data.target = reference_data.target.apply(
        lambda td: td.total_seconds() / 60)
    reference_data = reference_data[(
        reference_data.target >= 1) & (reference_data.target <= 60)]
...
def fetch_data():
    ...
    data = client.get_database(
        "prediction_service").get_collection("data").find()
...
@flow
def batch_analyze():
    upload_target("stream-generator/target.csv")
