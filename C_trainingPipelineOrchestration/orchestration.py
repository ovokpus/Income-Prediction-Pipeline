...
def train_best_model(X_train, y_train, X_val, y_val, dv):
    ...
    with mlflow.start_run():
        ...
        mlflow.log_artifact("./models/preprocessor.b", artifact_path=f"preprocessors")
        mlflow.xgboost.log_model(clf, artifact_path="models_mlflow")
...
def main_flow(
    trainpath: str = "./data/adult-train.csv", valpath: str = "./data/adult-val.csv"
):
    """
    ...
    # preprocess the data
    dv = DictVectorizer()
    X_train_preprocessed, dv = preprocess_data(X_train, dv, fit_dv=True).result()
    X_val_preprocessed = preprocess_data(X_val, dv).result()[0]

    # train the model
    best_clf = train_model_search(X_train, y_train, X_val, y_val).result()
    train_best_model(X_train, y_train, X_val, y_val, dv, wait_for=best_clf).result()
